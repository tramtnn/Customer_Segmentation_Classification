...
Sử dụng excel, SQL để xử lý dữ liệu và trích xuất dữ liệu. Dùng ngôn ngữ Python với mô hình RFM kết hợp Kmean để chọn ra nhóm khách hàng chính mà sàn thương mại điện tử cần quan tâm.
...
Use excel, SQL to process data and extract data. Use the Python language with the RFM model combined with Kmean to select the main customer groups that the e-commerce platform needs to care about.
